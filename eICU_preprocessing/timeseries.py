...
def rereconfigure_timeseries(timeseries, test=False):
    if test:
        timeseries = timeseries.iloc[300000:5000000]
    # convert index to multi-index with both patients and timedelta stamp

    timeseries.set_index(['patient',pd.to_timedelta(timeseries['time'])],inplace=True)
    timeseries.drop('time',axis=1, inplace=True)
    timeseries.index.rename(['patient','time'], inplace=True)
    return timeseries
...
def gen_timeseries_file(eICU_path, test=False):
    ...
    timeseries_lab.to_csv(eICU_path + 'reconfig_timeserieslab.csv')
    del(timeseries_lab)
    ...
    timeseries_resp.to_csv(eICU_path + 'reconfig_timeseriesresp.csv')
    del(timeseries_resp)
    ...
    timeseries_nurse.to_csv(eICU_path + 'reconfig_timeseriesnurse.csv')
    del(timeseries_nurse)
    ...
    timeseries_aperiodic = reconfigure_timeseries(timeseries_aperiodic,
                                                  ...
                                                  test=test)
    timeseries_aperiodic.to_csv(eICU_path + 'reconfig_timeseriesaperiodic.csv')
    ...
    timeseries_periodic.to_csv(eICU_path + 'reconfig_timeseriesperiodic.csv')
    del(timeseries_periodic)

    timeseries_lab = rereconfigure_timeseries(pd.read_csv(eICU_path + 'reconfig_timeserieslab.csv'))
    print('==> Loaded lab timeseries...')
    timeseries_resp = rereconfigure_timeseries(pd.read_csv(eICU_path + 'reconfig_timeseriesresp.csv'))
    print('==> Loaded resp timeseries...')
    timeseries_nurse = rereconfigure_timeseries(pd.read_csv(eICU_path + 'reconfig_timeseriesnurse.csv'))
    print('==> Loaded nurse timeseries...')
    timeseries_aperiodic = rereconfigure_timeseries(pd.read_csv(eICU_path + 'reconfig_timeseriesaperiodic.csv'))
    print('==> Loaded aperiodic timeseries...')
    timeseries_periodic = rereconfigure_timeseries(pd.read_csv(eICU_path + 'reconfig_timeseriesperiodic.csv'))
    print('==> Loaded periodic timeseries...')
    #size = 4000
    ...
    for patient_chunk in gen_chunks:
        ...
        resample_and_mask(merged, eICU_path, header, mask_decay=True, decay_rate=4/3, test=test, verbose=True)
